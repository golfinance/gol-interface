...
import TrainingCandidate from './TrainingCandidate'
...
const SelectNFT = ({ isOpen, closeDialog }) => {
  const { trainingNfts } = useContext(StakeContext)
  const [filteredMyNfts, setFilteredMyNfts] = useState(trainingNfts)
  ...
  const filterNFT = async (option, val) => {
    ...
    if (option === 'Position') {
      if (val === 'All') {
        tmpNfts = trainingNfts
      } else {
        for (let i = 0; i < trainingNfts.length; i++) {
          positions.push(getNFTPostion(trainingNfts[i].tokenId))
        }
        const result = await Promise.all(positions)
        for (let i = 0; i < result.length; i++) {
          if (result[i] === val) {
            tmpNfts.push(trainingNfts[i])
          }
        }
      }
  ...
  return (
    <Modal
      ...
      contentLabel="Training Selection Modal"
    >
      ...
      <div style={{ display: 'flex', width: '100%', flexWrap: 'wrap', justifyContent: 'center' }}>
        {_.map(filteredMyNfts, (nft) => (
          <TrainingCandidate data={nft} closeRequest={closeDialog} />
        ))}
      </div>
    </Modal>
  )
